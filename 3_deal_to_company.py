...
url = "https://api.hubapi.com/crm/v3/associations/deal/company/batch/create?hapikey=HS_API_KEY"
headers = {'Content-Type': "application/json"}

# SQL CODE
sql_query = '''SELECT * FROM frank.deals_companies;;'''

# Path and file name to log migrations
cwd = os.getcwd()
file_name = "deals_to_companies_log.txt"
complete_path = os.path.join(cwd + "/logs", file_name)

if __name__ == '__main__':
    # List of the rows of the errors (in 100s)
    row_errors = []
    # Get the data from PostGreSQL with keys that can be iterating by index or key
